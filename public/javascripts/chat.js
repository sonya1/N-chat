$(document).ready(function(){
    ...
    var socket = io.connect('http://localhost');  //连接服务器 无参数代表连接本地no 无参数不好使！！!
...
//发送用户上线信号
    ...
    function flushUsers(users){
        ...
        $("#list").empty().append('<li title="双击聊天" alt="all" class="sayingto" onselectstart="return false">所有人</li>'); //禁止选中文字
        //遍历生成用户在线列表
        for(var i in users){
        ...
        $("#list > li").dblclick(function(){
            ...
            if($(this).attr('alt')!=from){
                console.log("-->"+$(this).attr('alt')+"from-->"+from+".");
                //设置被双击的用户为说话对象
                to = $(this).attr('alt');
                //清除之前的选中效果
    ...
    $("#say").click(function(){
        ...
        if(to == "all"){
            $("#show").append('<div>你('+ now() +')对 所有人 说:<br/>'+ $msg +'</div>');
        }else{
            $("#show").append('<div style="color:#00f">你('+ now() +')对 '+ to+' 说:<br/>'+ $msg +'</div>');
        }
    ...
    socket.on('say',function(data){
        //对所有人说
        if(data.to == "all"){
            $("#show").append('<div style="color:#00f">'+ data.from +'('+ now() +')对 所有人 说:<br/>'+ data.msg +'</div>');
        }
        //对你密语
        if(data.to == from){
            $("#show").append('<div style="color:#00f">'+ data.from +'('+ now() +')对 你 说:<br/>'+ data.msg +'</div>');
        }
    });

    //服务器关闭
    socket.on('disconnect',function(){
        var sys = '<div style="color:#f00">系统：服务器连接失败！</div>';
        $("#show").append(sys+"<br>");
        $("#list").empty();
    });
    ...
    socket.on('reconnect',function(){
        var sys = '<div style="color:#f00">系统重新连接服务器！</div>';
        $("#show").append(sys+"<br>");
        socket.emit('online',{user:from});
    });
